  ...
  (lambda (d)
    (cond
      [(and (list? d) 
            (not (null? d))
            (symbol? (first d))
            (ast? (parse (second d))))(make-bind (parse (first d)) (parse (second d)))])))
  ...
  (lambda (d)
    (cond
      ...
      [(and
         ...
         (not (null? d))
         (eqv? (first d) 'if))
       (if-ast (parse (second d)) (parse (third d)) (parse (fourth d)))]
      [(and
        (list? d)
         (not (null? d))
         (eqv? (first d) 'cond))
       (if (eq? (first (second d)) 'else) (second (second d)) (if-ast (parse (first (second d))) (parse (second (second d))) (if (eq? (first (first (rest (rest d)))) 'else) (parse (second (first (rest (rest d))))) (parse (append '(cond) (rest (rest d)))))))]
      [(and
       (list? d)
       (not (null? d))
       (eqv? (first d) 'assume))
      (assume (first d) (map parse (cadr d)) (parse (caddr d)))]
